...
2. Read the actual changes — do not rely on commit messages or the existing description alone:
   ```
   gh pr diff <number>
   ```
...
One line: the problem or goal.
Then 2-6 bullets, each naming a concrete change (file/function/endpoint/schema/behaviour).
